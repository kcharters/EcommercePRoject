...
export default {
  ...
  created () {
    const ref = firebase.firestore().collection('womenshoodies').doc(this.$route.params.id);
    ref.get().then((doc) => {
      if (doc.exists) {
        this.item = doc.data();
      } else {
  ...
  methods: {
    onSubmit (evt) {
      evt.preventDefault()
      const updateRef = firebase.firestore().collection('womenshoodies').doc(this.$route.params.id);
